...
import {
  SENTENCE_BUILDER_TEMPLATES,
  shuffleWords,
} from '@/features/practice/data/sentenceTemplates'
import { shuffleDeck } from '@/data'
import { emptyPracticeStats } from '@/shared/quiz/practiceStats'

const SESSION_SIZE = 8

function preparePuzzle(template) {
  return {
    ...template,
    shuffled: shuffleWords(template.words),
  }
}

export function useSentenceBuilderSession({ enabled, resetKey }) {
  ...
  const buildDeck = useCallback(() => {
    const shuffled = shuffleDeck(SENTENCE_BUILDER_TEMPLATES)
    return shuffled
      .slice(0, Math.min(SESSION_SIZE, shuffled.length))
      .map(preparePuzzle)
  }, [])

  const startSession = useCallback(
    (reuseDeck = null) => {
      const raw = reuseDeck ?? buildDeck()
      const nextDeck = raw.map((t) =>
        t.shuffled ? t : preparePuzzle(t),
      )
  ...
  const puzzle = !enabled || complete ? null : (deck[index] ?? null)
